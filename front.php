...
function TTstore_scripts() { 
	...
	wp_register_script( 'ttstoreexpand-script', plugin_dir_url( __FILE__ ).'/js/expand.js', '', '', true);
	...
	if($ttsliderenable == "1"){
		...
		wp_register_style( 'tt_slider_css', plugin_dir_url( __FILE__ ). '/css/'.$ttslidertheme.'-jquery-ui.css', '', '', true);
...
function store_items($used, $winkel, $searching)
{
	...
	{
		_e('No XML filled in yet please change the settings first.', 'tradetracker-store');
	} else {
...
function header_css_style() {
	...
	foreach ($multi as $multi_val){
		...
		$storename = tt_store_create_slug($multi_val->multiname);
...
function show_ttusersort($winkelvol)
{
	if(isset($_GET['multiorder'])){
		$multiorder = sanitize_text_field($_GET['multiorder']);		
	} else {
		$multiorder = "";
	}
	...
	$userperpage .= __('Sort: ','tradetracker-store');
	...
	$userperpage .= __('price (high to low)','tradetracker-store');
	$userperpage .= "</option>";
	...
	$userperpage .= __('price (low to high)','tradetracker-store');
	$userperpage .= "</option>";
	$userperpage .= "</select>";
	$userperpage .= "</form>";
	return $userperpage;
...
function show_ttfilter($winkelvol)
{
	...
	if(isset($_GET['multisorting'])){
		$multisorting = "&multisorting=".sanitize_text_field($_GET['multisorting']);
	} else {
		$multisorting = "";
	}
	if(isset($_GET['multiorder'])){
		$multiorder = "&multiorder=".sanitize_text_field($_GET['multiorder']);
	} else {
		$multiorder = "";
	}
...
<p>
	<label for=\"amount\">".__('Price range:','tradetracker-store')."</label>
...
function show_ttpages($winkelvol)
{
	...
	foreach ($multi as $multi_val){	
		...
		if(count($nonexisting)>0){
			$Tradetracker_productid1 = array();
			foreach($nonexisting as $term){
				$Tradetracker_productid1[] = $term->productID;
			}
			$Tradetracker_productid1 = implode(",", $Tradetracker_productid1);
		}
				$productID = $Tradetracker_productid1;
				$productID = str_replace(",", "' or ".$ttstoretable.".productID='", $productID);
		

				$totalitems=count($wpdb->get_results("SELECT id FROM ".$ttstoretable.", ".$ttstorecattable." where ".$ttstorecattable.".productID = ".$ttstoretable.".productID and (".$ttstoretable.".productID='".$productID."') ".$multixmlfeed." ".$categorieselect." ".$priceselectcur." group by ".$ttstoretable.".productID ORDER BY ".$multisorting." ".$multiorder." ".$Tradetracker_amount_i.""));
			}
			...
			if($pages > "0"){
				...
				if ($currentpage != 0) { // Don't show back link if current page is first page.
					...
					$pagetext = "<a class=\"ttbackpage\" href=\"?ipp=".$itemsperpage."&tsp=".$back_page."".$multisorting."".$multiorder."".$min_price."".$max_pricecur."\">".__('back','tradetracker-store')."</a>\n";			
				}
				for ($i=0; $i <= $pages; $i++){
					if ($i == $currentpage){
				...
				if ($currentpage < $pages ) { // If last page don't give next link.
					...
					$pagetext .= "<a class=\"ttnextpage\" href=\"?ipp=".$itemsperpage."&tsp=".$next_page."".$multisorting."".$multiorder."".$min_price."".$max_pricecur."\">".__('next','tradetracker-store')."</a>\n";
				}
			}
		}
	}}
	...
	if ($searching == "1") {
		...
		if($multi_val->multipageamount > "0" || (isset($_GET['ipp']) && is_numeric($_GET['ipp']) && $_GET['ipp']>"0")){
			if (!isset($Tradetracker_productid) || $Tradetracker_productid == null) 
			{
				...
				$productID = str_replace(",", "' or ".$ttstoretable.".productID='", $productID);
				$totalitems=count($wpdb->get_results("SELECT id FROM ".$ttstoretable.", ".$ttstorecattable." where ".$ttstorecattable.".productID = ".$ttstoretable.".productID and (".$ttstoretable.".productID='".$productID."') ".$multixmlfeed." ".$categorieselect." ".$priceselectcur." group by ".$ttstoretable.".productID ".$Tradetracker_amount_i.""));
			}
			if(isset($_GET['ipp']) && is_numeric($_GET['ipp']) && $_GET['ipp']>"0"){
				$itemsperpage = ttstore_sanitize($_GET['ipp']);
			} else {
		...
		if($multisorting == "rand()"){
			if ($searching == "1") {
				...
				if (!isset($Tradetracker_productid) || $Tradetracker_productid == null) {
					if((isset($multixmlfeed) && $multixmlfeed != "") || (isset($categorieselect) && $categorieselect != "") || (isset($priceselect) && $priceselect != "")){
						$wherequery = " ".$multixmlfeed." ".$categorieselect." ".$priceselect;
					} else {
						$wherequery = "";
					}
					$visits=$wpdb->get_results("SELECT * FROM ".$ttstorecattable.", ".$ttstoretable." as t JOIN (SELECT(FLOOR(max(id) * rand())) as maxid FROM ".$ttstoretable.", ".$ttstorecattable." where ".$ttstorecattable.".productID = ".$ttstoretable.".productID ".$wherequery.") as tt on t.id >= tt.maxid where ".$ttstorecattable.".productID = t.productID $wherequery  group by t.productID ".$Tradetracker_amount_i."");
				} else {
					$productID = $Tradetracker_productid;
					$productID1 = str_replace(",", "' or t.productID='", $productID);
					$productID = str_replace(",", "' or ".$ttstoretable.".productID='", $productID);
					if(isset($productID) && $productID != ""){
						$wherequery = "where (".$ttstoretable.".productID='".$productID."') ".$multixmlfeed." ".$categorieselect." ".$priceselectcur;
						$wherequery1 = "where (t.productID='".$productID1."') ".$multixmlfeed." ".$categorieselect." ".$priceselectcur;
					} else {
						$wherequery = "where 1=1 ".$multixmlfeed." ".$categorieselect." ".$priceselectcur;
					}
					$visits=$wpdb->get_results("SELECT * FROM ".$ttstorecattable.", ".$ttstoretable." as t JOIN (SELECT(FLOOR(max(id) * rand())) as maxid FROM ".$ttstoretable.", ".$ttstorecattable." $wherequery ".$Tradetracker_amount_i.") as tt on t.id >= tt.maxid ".$wherequery1." and ".$ttstorecattable.".productID = t.productID group by t.productID  ".$Tradetracker_amount_i."");
				}
			}
			...
			if ($searching == "1") {
				...
				if (!isset($Tradetracker_productid) || $Tradetracker_productid == null) {
					...
					$productID = $Tradetracker_productid;
					$productID = str_replace(",", "' or ".$ttstoretable.".productID='", $productID);
					$visits=$wpdb->get_results("SELECT * FROM ".$ttstoretable.", ".$ttstorecattable." where ".$ttstorecattable.".productID = ".$ttstoretable.".productID and (".$ttstoretable.".productID='".$productID."') ".$multixmlfeed." ".$categorieselect." ".$priceselectcur." group by ".$ttstoretable.".productID ORDER BY ".$multisorting." ".$multiorder." ".$Tradetracker_amount_i."");
				}
			}
		}
	...
	foreach ($visits as $product){
		...
		if(!empty($Tradetracker_extra_val)){
			...
			if($extraname != ""){
				...
				$more = "<div class=\"".$storename."store-more store-more\"></div>";
			}
		} else {
			$more = "<div class=\"".$storename."store-more store-more\"></div>";
		}
		...
		$storeitems .= "<div class=\"".$storename."store-outerbox store-outerbox\">
				<a class=\"".$storename."store-titel store-titel\" href=\"".$producturl."\" ".$relurl." class=\"regular\" ".$urltarget." title=\"".$productname."\">
					".$productname."
				</a>
				".$logo."
				<div class=\"".$storename."store-image store-image\">
					<a href=\"".$image."\" ".$relimage." ".$imagerel." ".$target.">
				...
				<div class=\"".$storename."store-footer store-footer\">
					...
					<div class=\"".$storename."buttons buttons\">
						<a href=\"".$producturl."\" ".$relurl." class=\"regular\" ".$urltarget." title=\"".$productname."\">
							".$buynow."
						</a>
					</div>
