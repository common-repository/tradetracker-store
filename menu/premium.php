...
<div id="TB_window1" style="left: auto;margin-left: auto;margin-right: auto; margin-top: 0;right: auto;top: 48px;visibility: visible;z-index:100051;width: <?php echo esc_attr($adminwidth); ?>px;">
	...
	<?php echo $ttstorehidden; ?>
		<div id="TB_title">
			<div id="TB_ajaxWindowTitle"><?php _e('Insert the addon apikeys.','tradetracker-store'); ?></div>
	...
	<?php
		...
		$update = get_option('Tradetracker_premiumaccepted');
			if($update[$key]== "1") {
				...
				$accepted =  sprintf(__('buy an APIKey for %1$s <a href="%2$s" target="_blank">here</a>','tradetracker-store'),$key,$value);
			} else {
				$accepted =  sprintf(__('buy an APIKey for %1$s <a href="%2$s" target="_blank">here</a>','tradetracker-store'),$key,$value);
			}
	...
	?>
		<tr><td colspan="2"><hr><b><?php _e('Add extra functions','tradetracker-store'); ?></b></td></tr>
	<?php
		$i="50";
