		...
		if($Tradetracker_width_val=="" || $Tradetracker_layoutname_val ==""){
			...
			if(!empty($_POST['layoutid'])) {
		 		if ( $db_width_val  != $Tradetracker_width_val) {
					$query = $wpdb->update( $ttstorelayouttable, array( 'laywidth' => $Tradetracker_width_val), array( 'id' => intval($_POST['layoutid'])), array( '%s'), array( '%s'), array( '%d' ) );
  				}
 				if ( $db_layoutname_val  != $Tradetracker_layoutname_val) {
					$query = $wpdb->update( $ttstorelayouttable, array( 'layname' => $Tradetracker_layoutname_val), array( 'id' => intval($_POST['layoutid'])), array( '%s'), array( '%s'), array( '%d' ) );
  				}
				if ( $db_fontsize_val  != $Tradetracker_fontsize_val) {
					$query = $wpdb->update( $ttstorelayouttable, array( 'layfontsize' => $Tradetracker_fontsize_val), array( 'id' => intval($_POST['layoutid'])), array( '%s'), array( '%s'), array( '%d' ) );
				}
				if ( $db_font_val  != $Tradetracker_font_val) {
					$query = $wpdb->update( $ttstorelayouttable, array( 'layfont' => $Tradetracker_font_val), array( 'id' => intval($_POST['layoutid'])), array( '%s'), array( '%s'), array( '%d' ) );
				}
				if ( $db_colortitle_val  != $Tradetracker_colortitle_val) {
					$query = $wpdb->update( $ttstorelayouttable, array( 'laycolortitle' => $Tradetracker_colortitle_val), array( 'id' => intval($_POST['layoutid'])), array( '%s'), array( '%s'), array( '%d' ) );
  				}	
 				if ( $db_colorfooter_val  != $Tradetracker_colorfooter_val) {
					$query = $wpdb->update( $ttstorelayouttable, array( 'laycolorfooter' => $Tradetracker_colorfooter_val), array( 'id' => intval($_POST['layoutid'])), array( '%s'), array( '%s'), array( '%d' ) );
 				}
				if ( $db_colorimagebg_val  != $Tradetracker_colorimagebg_val) {
					$query = $wpdb->update( $ttstorelayouttable, array( 'laycolorimagebg' => $Tradetracker_colorimagebg_val), array( 'id' => intval($_POST['layoutid'])), array( '%s'), array( '%s'), array( '%d' ) );
				}
				if ( $db_colorfont_val  != $Tradetracker_colorfont_val) {
					$query = $wpdb->update( $ttstorelayouttable, array( 'laycolorfont' => $Tradetracker_colorfont_val), array( 'id' => intval($_POST['layoutid'])), array( '%s'), array( '%s'), array( '%d' ) );
				}
				if ( $db_colorborder_val  != $Tradetracker_colorborder_val) {
					$query = $wpdb->update( $ttstorelayouttable, array( 'laycolorborder' => $Tradetracker_colorborder_val), array( 'id' => intval($_POST['layoutid'])), array( '%s'), array( '%s'), array( '%d' ) );
				}
				if ( $db_colorbutton_val  != $Tradetracker_colorbutton_val) {
					$query = $wpdb->update( $ttstorelayouttable, array( 'laycolorbutton' => $Tradetracker_colorbutton_val), array( 'id' => intval($_POST['layoutid'])), array( '%s'), array( '%s'), array( '%d' ) );
				}
				if ( $db_colorbuttonfont_val  != $Tradetracker_colorbuttonfont_val) {
					$query = $wpdb->update( $ttstorelayouttable, array( 'laycolorbuttonfont' => $Tradetracker_colorbuttonfont_val), array( 'id' => intval($_POST['layoutid'])), array( '%s'), array( '%s'), array( '%d' ) );
				}
			} else {
				//save new variables
...
<div id="TB_window1" style="left: auto;margin-left: auto;margin-right: auto; margin-top: 0;right: auto;top: 48px;visibility: visible;z-index:100051;width: <?php echo esc_attr($adminwidth); ?>px;">
	<div id="ttstorebox">
		<div id="TB_title">
			<div id="TB_ajaxWindowTitle"><?php _e('Would you like to edit or add a layout?', 'tradetracker-store'); ?></div>
		...
		<table width="<?php echo esc_attr($adminwidth-15); ?>">
			<tr>
				...
				<td colspan="7">
				<?php _e('Colors:','tradetracker-store'); ?>
				</td>
			</tr>
			<tr>
				<td>
					<strong><?php _e('Name', 'tradetracker-store'); ?></strong>
				</td>
				<td>
					<strong><?php _e('Width', 'tradetracker-store'); ?></strong>
				</td>
				<td>
					<strong><?php _e('Font', 'tradetracker-store'); ?></strong>
				</td>
				<td>
					<strong><?php _e('Fontsize', 'tradetracker-store'); ?></strong>
				</td>
				<td>
					<strong><?php _e('Title', 'tradetracker-store'); ?></strong>
				</td>
				<td>
					<strong><?php _e('Image', 'tradetracker-store'); ?></strong>
				</td>
				<td>
					<strong><?php _e('Footer', 'tradetracker-store'); ?></strong>
				</td>
				<td>
					<strong><?php _e('Font', 'tradetracker-store'); ?></strong>
				</td>
				<td>
					<strong><?php _e('Border', 'tradetracker-store'); ?></strong>
				</td>
				<td>
					<strong><?php _e('Button', 'tradetracker-store'); ?></strong>
				</td>
				<td>
					<strong><?php _e('Button font', 'tradetracker-store'); ?></strong>
				</td>
				<td>
				</td>
			</tr>
...
?>
		...
		<div id="ttstoreboxbottom">
			<INPUT type="button" name="Close" class="button-primary" value="<?php _e('Add New','tradetracker-store'); ?>" onclick="location.href='admin.php?page=tt-store&option=layout&function=new'"> 
...
<div id="TB_window1" style="left: auto;margin-left: auto;margin-right: auto; margin-top: 0;right: auto;top: 48px;visibility: visible;z-index:100051;width: <?php echo esc_attr($adminwidth); ?>px;">
	...
	<?php echo $ttstorehidden; ?>
		<div id="TB_title">
			<div id="TB_ajaxWindowTitle"><?php if(isset($layoutid)){ _e('Edit layout', 'tradetracker-store'); } else { _e('Create layout', 'tradetracker-store'); } ?></div>
			<div id="TB_closeAjaxWindow">
...
<table width="<?php echo esc_attr($adminwidth-15); ?>">
	<tr>
		<td>
			<label for="tradetrackername" title="<?php _e("Fill in the name for the layout.", 'tradetracker-store' ); ?>" class="info">
				<?php _e("Name for Layout:", 'tradetracker-store' ); ?>
			</label> 
		</td>
		<td>
	...
	<tr>
		<td>
			<label for="tradetrackerwidth" title="<?php _e("Fill in which width you want 1 item to have.", 'tradetracker-store' ); ?>" class="info">
				<?php _e("Store width:", 'tradetracker-store' ); ?>
			</label> 
		</td>
		<td>
	...
	<tr>
		<td>
			<label for="tradetrackerfont" title="<?php _e("Fill in which font you want to use. Standard font is Verdana.", 'tradetracker-store' ); ?>" class="info">
				<?php _e("Font:", 'tradetracker-store' ); ?> 
			</label> 
		</td>
		<td>
	...
	<tr>
		<td>
			<label for="tradetrackerfontsize" title="<?php _e("Fill in which size the font should be. Standard is 10", 'tradetracker-store' ); ?>" class="info">
				<?php _e("Fontsize:", 'tradetracker-store' ); ?> 
			</label> 
		</td>
		<td>
	...
	<tr>
		<td>
			<label for="tradetrackercolortitle" title="<?php _e("What color would you like to use for your title background.", 'tradetracker-store' ); ?>" class="info">
				<?php _e("Title background color:", 'tradetracker-store' ); ?> 
			</label> 
		</td>
		<td>
	...
	<tr>
		<td>
			<label for="tradetrackercolorimagebg" title="<?php _e("What color would you like to use for your image background.", 'tradetracker-store' ); ?>" class="info">
				<?php _e("Image background color:", 'tradetracker-store' ); ?> 
			</label> 
		</td>
		<td>
	...
	<tr>
		<td>
			<label for="tradetrackercolorfooter" title="<?php _e("What color would you like to use for your footer background.", 'tradetracker-store' ); ?>" class="info">
				<?php _e("Footer background color:", 'tradetracker-store' ); ?> 
			</label> 
		</td>
		<td>
	...
	<tr>
		<td>
			<label for="tradetrackercolorfooter" title="<?php _e("What color would you like to use for the border.", 'tradetracker-store' ); ?>" class="info">
				<?php _e("Border color:", 'tradetracker-store' ); ?> 
			</label> 
		</td>
		<td>
	...
	<tr>
		<td>
			<label for="tradetrackercolorbutton" title="<?php _e("What color would you like to use for the button.", 'tradetracker-store' ); ?>" class="info">
				<?php _e("Button color:", 'tradetracker-store' ); ?> 
			</label> 
		</td>
		<td>
	...
	<tr>
		<td>
			<label for="tradetrackerbuttoncolorfont" title="<?php _e("What font color would you like to use for the button.", 'tradetracker-store' ); ?>" class="info">
				<?php _e("Button Font color:", 'tradetracker-store' ); ?> 
			</label> 
		</td>
		<td>
	...
	<tr>
		<td>
			<label for="tradetrackercolorfont" title="<?php _e("What font color would you like to use.", 'tradetracker-store' ); ?>" class="info">
				<?php _e("Font color:", 'tradetracker-store' ); ?> 
			</label> 
		</td>
		<td>
		...
		<div id="ttstoreboxbottom">
			...
			?>
			<input type="submit" name="Submit" class="button-primary" value="<?php if(isset($layoutid)){ esc_attr_e('Save Changes'); } else { _e('Create','tradetracker-store'); } ?>" /> 
