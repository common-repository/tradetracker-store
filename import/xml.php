...
function xml_updater($xmlfilecount = "-1", $xmlfeedID = "0", $xmlcronjob = "0") {
	...
	if ($xmlfeedID == "0" && isset($_GET['xmlfeedID'])){
		$xmlfeedID = intval($_GET['xmlfeedID']);
	}
	if (($xmlfilecount == "-1" && !isset($_GET['xmlfilecount'])) || $xmlfilecount == "-2"){
		...
		if ($xmlcronjob == "1"){
			...
			if( $autoimport_count ) {
				...
				if (get_option("TTDBUniqueupdate") != "1"){	
					$wpdb->query("ALTER TABLE `wp_tradetracker_store` DROP INDEX `productID`;");
					$wpdb->query("ALTER TABLE `wp_tradetracker_store` ADD UNIQUE( `productID`);");
					update_option("TTDBUniqueupdate", "1" );
				}
	...
	$runs = "0";
	$feedsimported = sprintf(__('<strong>Feeds Completed: </strong> %1$s / %2$s','tradetracker-store'), $runs, count($loadxmlfeeds));
	...
	if ($xmlfilecount <= count($loadxmlfeeds)-1){
		...
		call_user_func_array($value,  array($xmlfeedid, $basefilename, $key,$filenum,$recordnum,$processed,'products', 'itemXMLtag'));
		...
		$feedsimported = sprintf(__('<strong>Feeds Completed: </strong> %1$s / %2$s','tradetracker-store'), count($loadxmlfeeds), count($loadxmlfeeds));
		update_option( "Tradetracker_feedsimported", $feedsimported );
