...
function fill_database1($xmlfeedid, $xmlcronjob)
{
	...
	if(!isset($wpdb) || empty($wpdb)){
		tt_store_log_me("TT Database: wpdb not loaded");
	}
	global $errorfile;
	if(!isset($errorfile) || empty($errorfile)){
		tt_store_log_me("TT Database: errorfile not loaded");
	}
	GLOBAL $extrafield;
	if(!isset($extrafield) || empty($extrafield)){
		tt_store_log_me("TT Database: extrafield not loaded");
	}
	GLOBAL $extravalue;
	if(!isset($extravalue) || empty($extravalue)){
		tt_store_log_me("TT Database: extravalue not loaded");
	}
	GLOBAL $counterxml;
	if(!isset($counterxml) || empty($counterxml)){
		tt_store_log_me("TT Database: counterxml not loaded");
	}
	global $ttstoretable;
	if(!isset($ttstoretable) || empty($ttstoretable)){
		tt_store_log_me("TT Database: ttstoretable not loaded");
	}
	global $ttstoreextratable;
	if(!isset($ttstoreextratable) || empty($ttstoreextratable)){
		tt_store_log_me("TT Database: ttstoreextratable not loaded");
	}
	global $ttstorexmltable;
	if(!isset($ttstorexmltable) || empty($ttstorexmltable)){
		tt_store_log_me("TT Database: ttstorexmltable not loaded");
	}
	global $ttstorecattable;
	if(!isset($ttstorecattable) || empty($ttstorecattable)){
		tt_store_log_me("TT Database: ttstorecattable not loaded");
	}
	global $foldersplits;
	if(!isset($foldersplits) || empty($foldersplits)){
		tt_store_log_me("TT Database: foldersplits not loaded");
	}
	...
	if ($xmlcronjob == "0"){
		...
		tt_store_log_me("TT Database: After memory usage");
	if (is_array($files)) {
		tt_store_log_me("TT Database: Files is array");
		for ( $i="0"; ($i <= 2 && $i <count($files)); $i++) {
			...
			if($filename != ""){
				...
				if($products === false)
				{
					...
					libxml_clear_errors();
					tt_store_log_me("TT Database: Error 1");
					...
					libxml_clear_errors();
					update_option( "Tradetracker_importerror", $errorfile );
					tt_store_log_me("TT Database: Error 2");
				} else {
					tt_store_log_me("TT Database: No error");
					foreach($products as $product) // loop through our items
					{
						...
						$currentpage["xmlfeed"]=(string) $product->xmlfile;	
						$currentpage["name"]= (string) $product->name;
						//$currentpage["imageURL"]=$product->imageURL;
						$currentpage["imageURL"]=(string) $product->imageURL;
						$currentpage["productURL"]=(string) $product->productURL;
						$currentpage["description"]=strip_tags(htmlspecialchars_decode($product->description));
						$currentpage["price"]= (string) $product->price;
						$currentpage["currency"]= (string) $product->price['currency'];
						...
						if($product->categories->category==""){
							if (isset($newcategory) && $newcategory == "1"){ 
								...
								$wpdb->insert($ttstorecattable ,array('productID' => $productID,'categorie' => "empty category",'categorieid' => md5("empty category") ),array('%s','%s','%s'));
							}
						} else {
							$querycat = "";
							$i="1";
	...
	$newvalue = sprintf(__('Database filled with %1$s new items on %2$s','tradetracker-store'), $item_count, $currentupdate);
