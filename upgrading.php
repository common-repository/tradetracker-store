<?php
if (get_option("TTstoreversion") == "4.6.18"){
	update_option("Tradetracker_sliderenable", '1');
	update_option("TTstoreversion", "4.6.36" );
}
if (get_option("TTstoreversion") == "4.6.17"){
	global $wpdb;
	$TT_Storepro_table_prefix=$wpdb->prefix.'tradetracker_';
	$ttmultitable = $TT_Storepro_table_prefix."multi";
	$wpdb->query("ALTER TABLE `".$ttmultitable."` CHANGE `multiitems` `multiitems` varchar(10000) NULL");
	$wpdb->query("ALTER TABLE `".$ttmultitable."` CHANGE `categories` `categories` longtext NULL");
	$wpdb->query("ALTER TABLE `".$ttmultitable."` CHANGE `buynow` `buynow` TEXT NULL");
	$wpdb->query("ALTER TABLE `".$ttmultitable."` CHANGE `multiproductpage` `multiproductpage` VARCHAR(1) NULL DEFAULT '0'");
	update_option("TTstoreversion", "4.6.18" );
}

if (get_option("TTstoreversion") == "4.6.14"){
	global $wpdb;
	$TT_Storepro_table_prefix=$wpdb->prefix.'tradetracker_';
	$ttstoretable = $TT_Storepro_table_prefix."store";
	$wpdb->query("ALTER TABLE `".$ttstoretable."` CHANGE `currency` `currency` VARCHAR(10) NULL DEFAULT 'EUR'");
	update_option("TTstoreversion", "4.6.17" );
}
...
if (get_option("TTstoreversion") == "4.5.54"){
	global $wpdb;
	$TT_Storepro_table_prefix=$wpdb->prefix.'tradetracker_';
	$tttable = $TT_Storepro_table_prefix."store";
	$tttableextra = $TT_Storepro_table_prefix."extra";
	$tttablecat = $TT_Storepro_table_prefix."cat";
...
if (get_option("TTstoreversion") == "4.5.7"){
	global $wpdb;
	$TT_Storepro_table_prefix=$wpdb->prefix.'tradetracker_';
	$ttxmltable = $TT_Storepro_table_prefix."xml";
